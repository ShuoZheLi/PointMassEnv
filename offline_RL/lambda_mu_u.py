...
class VDICE:
    def __init__(
        ...
        self.mu = mu_network
        self.U = U_network
        self.old_mu = copy.deepcopy(mu_network).to(device)
        self.e_mu = copy.deepcopy(self.q).to(device)
        self.e_mu_bias_reduce = copy.deepcopy(self.q).to(device)
        self.e_mu_bias_reduce_u = copy.deepcopy(self.q).to(device)
        ...
        self.old_mu_optimizer = torch.optim.Adam(self.old_mu.parameters(), lr=3e-4)
        self.e_mu_optimizer = torch.optim.Adam(self.e_mu.parameters(), lr=3e-4)
        self.e_mu_bias_reduce_optimizer = torch.optim.Adam(self.e_mu_bias_reduce.parameters(), lr=3e-4)
        self.e_mu_bias_reduce_u_optimizer = torch.optim.Adam(self.e_mu_bias_reduce_u.parameters(), lr=3e-4)
        ...
        self.old_s_actor = copy.deepcopy(self.semi_sa_actor_and).to(device)
        self.old_s_actor_optimizer = torch.optim.Adam(self.old_s_actor.parameters(), lr=3e-4)
        self.old_s_actor_lr_schedule = CosineAnnealingLR(self.old_s_actor_optimizer, max_steps)

        self.e_mu_s_actor = copy.deepcopy(self.semi_sa_actor_and).to(device)
        self.e_mu_s_actor_optimizer = torch.optim.Adam(self.e_mu_s_actor.parameters(), lr=3e-4)
        self.e_mu_s_actor_lr_schedule = CosineAnnealingLR(self.e_mu_s_actor_optimizer, max_steps)

        self.e_mu_bias_reduce_s_actor = copy.deepcopy(self.semi_sa_actor_and).to(device)
        self.e_mu_bias_reduce_s_actor_optimizer = torch.optim.Adam(self.e_mu_bias_reduce_s_actor.parameters(), lr=3e-4)
        self.e_mu_bias_reduce_s_actor_lr_schedule = CosineAnnealingLR(self.e_mu_bias_reduce_s_actor_optimizer, max_steps)

        self.e_mu_bias_reduce_u_s_actor = copy.deepcopy(self.semi_sa_actor_and).to(device)
        self.e_mu_bias_reduce_u_s_actor_optimizer = torch.optim.Adam(self.e_mu_bias_reduce_u_s_actor.parameters(), lr=3e-4)
        self.e_mu_bias_reduce_u_s_actor_lr_schedule = CosineAnnealingLR(self.e_mu_bias_reduce_u_s_actor_optimizer, max_steps)
    ...
    def _update_mu(
        ...
        with torch.no_grad():
            ...
            semi_a_weight = (semi_a_weight - self.min_a_weight) / (self.max_a_weight - self.min_a_weight + 1e-6)
        ...
        semi_a_weighted_residual = self.semi_q_alpha * frenchel_dual(self.f_name, semi_a_weighted_residual / self.semi_q_alpha)

        mu_dual_loss = torch.mean(semi_a_weighted_residual)
        # mu_linear_loss = (1 - self.discount) * torch.mean(self.mu(init_observations))
        mu_residual_loss = torch.mean(-mu_residual)
        mu_loss = (1 - config.mu_lambda) * mu_residual_loss + config.mu_lambda * mu_dual_loss
    ...
    def _update_old_mu(
        ...
        # Calculate mu and mu_residual in a vectorized manner
        mu = self.old_mu(observations)
        mu_next = self.old_mu(next_observations)
        mu_residual = (1.0 - terminals.float()) * self.discount * mu_next - mu
        ...
        mu_dual_loss = torch.mean(semi_a_weighted_residual)
        # mu_linear_loss = (1 - self.discount) * torch.mean(self.mu(init_observations))
        mu_linear_loss = torch.mean(self.mu(init_observations))
        # mu_residual_loss = torch.mean(-mu_residual)
        mu_loss = (1 - config.mu_lambda) * mu_linear_loss + config.mu_lambda * mu_dual_loss

        self.old_mu_optimizer.zero_grad()
        mu_loss.backward()
        self.old_mu_optimizer.step()

        log_dict["vdice_loss/old_mu_loss"] = mu_loss.item()
        log_dict["value_train/old_mu_value"] = mu.mean().item()
    ...
    def _update_e_mu(
        ...
        # mu = self.mu(observations)
        # mu_next = self.mu(next_observations)
        mu_residual = self.e_mu(observations, actions)

        # Apply semi_a_weight to mu_residual
        semi_a_weighted_residual = mu_residual * semi_a_weight

        # Apply frenchel_dual in a vectorized way
        semi_a_weighted_residual = frenchel_dual(self.f_name, semi_a_weighted_residual)
        ...
        self.e_mu_optimizer.zero_grad()
        mu_loss.backward()
        self.e_mu_optimizer.step()

        log_dict["vdice_loss/e_mu"] = mu_loss.item()
        log_dict["value_train/e_mu"] = mu_residual.mean().item()
    
    def _update_e_mu_bias_reduce(
        self,
        observations: torch.Tensor,
        actions: torch.Tensor,
        next_observations: torch.Tensor,
        ...
        # mu = self.mu(observations)
        # mu_next = self.mu(next_observations)
        mu_residual = self.e_mu_bias_reduce(observations, actions)

        # Apply semi_a_weight to mu_residual
        semi_a_weighted_residual = mu_residual * semi_a_weight

        with torch.no_grad():
            sg_term = frenchel_dual_prime(self.f_name, semi_a_weighted_residual)


        # Apply frenchel_dual in a vectorized way
        semi_a_weighted_residual = sg_term * semi_a_weighted_residual
        ...
        self.e_mu_bias_reduce_optimizer.zero_grad()
        mu_loss.backward()
        self.e_mu_bias_reduce_optimizer.step()

        log_dict["vdice_loss/e_mu_bias_reduce"] = mu_loss.item()
        log_dict["value_train/e_mu_bias_reduce"] = mu_residual.mean().item()

    def _update_u_e_mu_bias_reduce(
        self,
        observations: torch.Tensor,
        actions: torch.Tensor,
        next_observations: torch.Tensor,
        ...
        mu_next = self.mu(next_observations)
        mu_residual = (1.0 - terminals.float()) * self.discount * mu_next - mu
        # mu_residual = self.e_mu_bias_reduce_u(observations, actions)
        ...
        with torch.no_grad():
            u = self.U(observations)
            sg_term = frenchel_dual_prime(self.f_name, u)
        ...
        # self.e_mu_bias_reduce_u_optimizer.zero_grad()
        self.mu_optimizer.zero_grad()
        mu_loss.backward()
        # self.e_mu_bias_reduce_u_optimizer.step()
        self.mu_optimizer.step()

        log_dict["vdice_loss/e_mu_bias_reduce_u"] = mu_loss.item()
        log_dict["value_train/e_mu_bias_reduce_u"] = mu_residual.mean().item()
    ...
    def _update_U(
        ...
        with torch.no_grad():
            ...
            semi_a_weight = f_prime_inverse(self.f_name, adv)
            semi_a_weight = (semi_a_weight - self.min_a_weight) / (self.max_a_weight - self.min_a_weight + 1e-6)
            u_target = semi_a_weight * mu_residual

        
    
        semi_a_weighted_residual = self.U(observations)
        u_loss = F.mse_loss(semi_a_weighted_residual, u_target)
        ...
        log_dict["value_train/U_value"] = semi_a_weighted_residual.mean().item()
    ...
    def _update_policy(
        ...
        semi_s_weight, semi_a_weight, old_semi_s_weight, semi_s_e_mu_weight, semi_s_e_mu_bias_reduce_weight, semi_s_e_mu_bias_reduce_u_weight, true_sa_weight = self.get_weights(observations, 
                                                                        next_observations, 
                                                                        actions, 
                                                                        rewards, 
                                                                        terminals)
        ...
        if self.total_it >= config.pi_ratio_step:
            ...
            # policy_out = self.old_s_actor(observations)
            # bc_losses = -policy_out.log_prob(actions).sum(-1, keepdim=False)
            # policy_loss = torch.mean(old_semi_s_weight * bc_losses)
            # self.old_s_actor_optimizer.zero_grad()
            # policy_loss.backward()
            # self.old_s_actor_optimizer.step()
            # self.old_s_actor_lr_schedule.step()
            # log_dict["vdice_loss/old_semi_s_policy_loss"] = policy_loss.item()

            # policy_out = self.e_mu_s_actor(observations)
            # bc_losses = -policy_out.log_prob(actions).sum(-1, keepdim=False)
            # policy_loss = torch.mean(semi_s_e_mu_weight * bc_losses)
            # self.e_mu_s_actor_optimizer.zero_grad()
            # policy_loss.backward()
            # self.e_mu_s_actor_optimizer.step()
            # self.e_mu_s_actor_lr_schedule.step()
            # log_dict["vdice_loss/semi_s_e_mu_bias_reduce_policy_loss"] = policy_loss.item()

            # policy_out = self.e_mu_bias_reduce_s_actor(observations)
            # bc_losses = -policy_out.log_prob(actions).sum(-1, keepdim=False)
            # policy_loss = torch.mean(semi_s_e_mu_bias_reduce_weight * bc_losses)
            # self.e_mu_bias_reduce_s_actor_optimizer.zero_grad()
            # policy_loss.backward()
            # self.e_mu_bias_reduce_s_actor_optimizer.step()
            # self.e_mu_bias_reduce_s_actor_lr_schedule.step()
            # log_dict["vdice_loss/semi_s_e_mu_bias_reduce_policy_loss"] = policy_loss.item()

            policy_out = self.e_mu_bias_reduce_u_s_actor(observations)
            bc_losses = -policy_out.log_prob(actions).sum(-1, keepdim=False)
            policy_loss = torch.mean(semi_s_e_mu_bias_reduce_u_weight * bc_losses)
            self.e_mu_bias_reduce_u_s_actor_optimizer.zero_grad()
            policy_loss.backward()
            self.e_mu_bias_reduce_u_s_actor_optimizer.step()
            self.e_mu_bias_reduce_u_s_actor_lr_schedule.step()
            log_dict["vdice_loss/semi_s_e_mu_bias_reduce_u_policy_loss"] = policy_loss.item()
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        if self.total_it >= config.pi_ratio_step:
            ...
            # self._update_old_mu(observations,
            #                 actions,
            #                 next_observations, 
            #                 dones, 
            #                 init_observations,
            ...
            # self._update_e_mu(observations,
            #                 actions,
            #                 next_observations, 
            #                 dones, 
            #                 init_observations,
            ...
            # self._update_e_mu_bias_reduce(observations,
            #                               actions,
            #                                 next_observations,
            #                                 dones,
            #                                 init_observations,
            ...
            self._update_u_e_mu_bias_reduce(observations,
                                            actions,
                                            next_observations,
                                            dones,
                                            init_observations,
    ...
    def get_weights(
        ...
        with torch.no_grad():
            ...
            mu_next = self.old_mu(next_observations)
            mu_residual = (1.0 - terminals.float()) * self.discount * mu_next - mu
            old_semi_s_weight = f_prime_inverse(self.f_name, semi_a_weight * mu_residual)


            mu = self.e_mu(observations, actions)
            semi_s_e_mu = f_prime_inverse(self.f_name, semi_a_weight * mu)

            mu_residual = self.e_mu_bias_reduce(observations, actions)
            semi_s_e_mu_bias_reduce_weight = f_prime_inverse(self.f_name, semi_a_weight * mu_residual)
            


            semi_s_e_mu_bias_reduce_u_weight = f_prime_inverse(self.f_name, self.U(observations))
            ...
            true_sa_weight = semi_s_e_mu_bias_reduce_u_weight * semi_a_weight
        
        return semi_s_weight, semi_a_weight, old_semi_s_weight, semi_s_e_mu, semi_s_e_mu_bias_reduce_weight, semi_s_e_mu_bias_reduce_u_weight, true_sa_weight
...
def trainer_init(config: TrainConfig, env):
    ...
    if config.checkpoints_path is not None:
        ...
        os.makedirs(config.checkpoints_path+"/gif/old_semi_s_weight", exist_ok=True)
        os.makedirs(config.checkpoints_path+"/gif/e_mu_weight", exist_ok=True)
        os.makedirs(config.checkpoints_path+"/gif/semi_s_e_mu_bias_reduce_weight", exist_ok=True)
        os.makedirs(config.checkpoints_path+"/gif/semi_s_e_mu_bias_reduce_u_weight", exist_ok=True)

        os.makedirs(config.checkpoints_path+"/gif/old_semi_s", exist_ok=True)
        ...
        os.makedirs(config.checkpoints_path+"/gif/true_sa", exist_ok=True)
...
def create_dataset(config: TrainConfig):
    ...
    dataset = np.load("PointMassEnv/EmptyRoom/noisey/dataset.npy", allow_pickle=True)
...
def get_weights(dataset, trainer):
    ...
    old_semi_s_weight = []
    semi_s_e_mu_weight = []
    semi_s_e_mu_bias_reduce_weight = []
    semi_s_e_mu_bias_reduce_u_weight = []
    semi_a_weight = []
    true_sa_weight = []
    ...
    for i in range(0,len(dataset["observations"]),32768):
        ...
        semi_s, semi_a, old_semi_s, semi_s_e_mu, semi_s_e_mu_bias_reduce, semi_s_e_mu_bias_reduce_u, true_sa = trainer.get_weights(obs, 
                                                        next_obs, 
                                                        actions, 
                                                        rewards, 
                                                        terminals)
        semi_s_weight.append(semi_s.cpu().numpy())
        old_semi_s_weight.append(old_semi_s.cpu().numpy())
        semi_s_e_mu_weight.append(semi_s_e_mu.cpu().numpy())
        semi_s_e_mu_bias_reduce_weight.append(semi_s_e_mu_bias_reduce.cpu().numpy())
        semi_s_e_mu_bias_reduce_u_weight.append(semi_s_e_mu_bias_reduce_u.cpu().numpy())
    ...
    old_semi_s_weight = np.hstack(old_semi_s_weight)

    semi_s_e_mu_weight = np.hstack(semi_s_e_mu_weight)

    semi_s_e_mu_bias_reduce_weight = np.hstack(semi_s_e_mu_bias_reduce_weight)

    semi_s_e_mu_bias_reduce_u_weight = np.hstack(semi_s_e_mu_bias_reduce_u_weight)
    ...
    return semi_s_weight, semi_a_weight, old_semi_s_weight, semi_s_e_mu_weight, semi_s_e_mu_bias_reduce_weight, semi_s_e_mu_bias_reduce_u_weight, true_sa_weight
...
def train():
    ...
    config.load_model = "mu_loss_study_normal_a_weight/mu_loss_study_normal_a_weight_semi_dice_lambda_0_6_semi_q_alpha_1_0/model/checkpoint_989999.pt"
    config.load_yaml = "mu_loss_study_normal_a_weight/mu_loss_study_normal_a_weight_semi_dice_lambda_0_6_semi_q_alpha_1_0/config.yaml"
    ...
    while t < int(config.max_timesteps):
        ...
        if semi_trainer.total_it + 1 == config.pi_ratio_step:
            ...
            semi_s_weight, semi_a_weight, old_semi_s_weight, e_mu_weight, semi_s_e_mu_bias_reduce_weight, semi_s_e_mu_bias_reduce_u_weight, true_sa_weight = get_weights(dataset, semi_trainer)
            semi_trainer.max_a_weight = np.max(semi_a_weight)
            semi_trainer.min_a_weight = np.min(semi_a_weight)
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            policy_log_dict["policy_train/old_semi_s_perform"], \
            policy_log_dict["policy_train/old_semi_s_epi_len"], \
            old_semi_s_traj = eval_policy(semi_trainer.old_s_actor, semi_trainer.total_it, config.checkpoints_path+"/gif/old_semi_s_weight", config.device, wandb, "old_semi_s_perform")

            policy_log_dict["policy_train/e_mu_perform"], \
            policy_log_dict["policy_train/e_mu_epi_len"], \
            e_mu_traj = eval_policy(semi_trainer.e_mu_s_actor, semi_trainer.total_it, config.checkpoints_path+"/gif/e_mu_weight", config.device, wandb, "e_mu_s_reduce_perform")

            policy_log_dict["policy_train/semi_s_e_mu_bias_reduce_perform"], \
            policy_log_dict["policy_train/semi_s_e_mu_bias_reduce_epi_len"], \
            semi_s_e_mu_bias_reduce_traj = eval_policy(semi_trainer.e_mu_bias_reduce_s_actor, semi_trainer.total_it, config.checkpoints_path+"/gif/semi_s_e_mu_bias_reduce_weight", config.device, wandb, "semi_s_e_mu_bias_reduce_perform")

            policy_log_dict["policy_train/semi_s_e_mu_bias_reduce_u_perform"], \
            policy_log_dict["policy_train/semi_s_e_mu_bias_reduce_u_epi_len"], \
            semi_s_e_mu_bias_reduce_u_traj = eval_policy(semi_trainer.e_mu_bias_reduce_u_s_actor, semi_trainer.total_it, config.checkpoints_path+"/gif/semi_s_e_mu_bias_reduce_u_weight", config.device, wandb, "semi_s_e_mu_bias_reduce_u_perform")
        ...
        if (t + 1) % config.save_freq == 0:
            if config.checkpoints_path is not None:
                ...
                semi_s_weight, semi_a_weight, old_semi_s_weight, e_mu_weight, semi_s_e_mu_bias_reduce_weight, semi_s_e_mu_bias_reduce_u_weight, true_sa_weight = get_weights(temp_dataset, semi_trainer)
                # weights_dict = {
                ...
                semi_s_img = draw_traj(semi_s_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=semi_s_traj)
                semi_a_img = draw_traj(semi_a_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=semi_a_traj)
                old_semi_s_img = draw_traj(old_semi_s_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=old_semi_s_traj)
                e_mu_img = draw_traj(e_mu_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=e_mu_traj)
                semi_s_e_mu_bias_reduce_img = draw_traj(semi_s_e_mu_bias_reduce_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=semi_s_e_mu_bias_reduce_traj)
                semi_s_e_mu_bias_reduce_u_img = draw_traj(semi_s_e_mu_bias_reduce_u_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=semi_s_e_mu_bias_reduce_u_traj)
                bc_img = draw_traj(np.ones_like(semi_s_weight), temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=bc_traj)
                true_s_and_a_img = draw_traj(true_sa_weight, temp_dataset, env, save_path = save_dir + "/" + str(semi_trainer.total_it) + ".png", trajectories=true_s_and_a_traj)

                wandb.log({"selected_traj/" + "semi_s": wandb.Image(semi_s_img),
                           "selected_traj/" + "semi_a": wandb.Image(semi_a_img),
                           "selected_traj/" + "old_semi_s": wandb.Image(old_semi_s_img),
                           "selected_traj/" + "e_mu": wandb.Image(e_mu_img),
                           "selected_traj/" + "semi_s_e_mu_bias_reduce": wandb.Image(semi_s_e_mu_bias_reduce_img),
                           "selected_traj/" + "semi_s_e_mu_bias_reduce_u": wandb.Image(semi_s_e_mu_bias_reduce_u_img),
                           "selected_traj/" + "bc": wandb.Image(bc_img),
                            "selected_traj/" + "true_s_and_a": wandb.Image(true_s_and_a_img),
                           "selected_traj_step" : semi_trainer.total_it,
                           },)
