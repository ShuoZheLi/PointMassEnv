...
class VDICE:
    ...
    #     semi_q = self.semi_q.both(observations, actions)
    #     adv = targets - semi_q
    #     # forward_dual_loss = torch.mean(self.semi_q_alpha * frenchel_dual_q(self.f_name, adv / self.semi_q_alpha))
    #     forward_dual_loss = torch.mean(self.semi_q_alpha * frenchel_dual(self.f_name, adv / self.semi_q_alpha))
    ...
    #     adv = targets - semi_q
    #     # forward_dual_loss = torch.mean(self.semi_q_alpha * frenchel_dual_q(self.f_name, adv / self.semi_q_alpha))
    #     forward_dual_loss = torch.mean(self.semi_q_alpha * frenchel_dual(self.f_name, semi_a_weight * adv / self.semi_q_alpha))
    ...
    def _update_semi_q(
        ...
        with torch.no_grad():
            init_a = self.semi_a_actor(init_observations).mean
            next_a = self.semi_a_actor(next_observations).mean
            # init_a = self.bc_actor(init_observations).mean
            # next_a = self.bc_actor(next_observations).mean
