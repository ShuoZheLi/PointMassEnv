...
def draw_traj(weights, dataset, env, save_path=None, trajectories=None, values=None):
    ...
    selected_traj_img = env.get_env_frame_with_selected_traj_plt_val_num(obs=selected_obs, 
                                                            next_obs=selected_next_obs,
                                                            terminals=selected_terminals,
                                                            trajectories=trajectories,
                                                            values=values,
...
def main(config: TrainConfig):
    ...
    file_name = "mature_v_q/mature_v_q_normalize_reward_True_true_dice_alpha_1_discount_0_99_semi_dice_lambda_0_6_seed_19990526_percent_expert_0_eval_freq_5000_save_freq_5000_batch_size_256_hidden_dim_256/test-antmaze-umaze-v2-8b8b1d70/config.yaml"
    ...
    trainer.load_state_dict(
            torch.load("mature_v_q/mature_v_q_normalize_reward_True_true_dice_alpha_1_discount_0_99_semi_dice_lambda_0_6_seed_19990526_percent_expert_0_eval_freq_5000_save_freq_5000_batch_size_256_hidden_dim_256/test-antmaze-umaze-v2-8b8b1d70/model/checkpoint_539999.pt")
            )
    state_value, action_state_value, true_state_value = trainer.get_value(env=env)

    

    weights = np.array([0], dtype=np.bool) * dataset["observations"].shape[0]
    selected_img  = draw_traj(weights, dataset, env, save_path = "value_map.png", trajectories=None, values=action_state_value)
